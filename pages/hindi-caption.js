...
const HindiCaption = (props) => {
  const [hindiCaption, setHindiCaption] = useState(props.HindiCaption);
  return (
    <>
      <Head>
        <title>IG-Instagram-Caption in Hindi</title>
        <meta
          name="description"
        ...
        <meta
          name="keywords"
          content="Status in hindi, Caption in hindi, Instagram Caption, Caption, Status, Whatsapp status, Whatsapp caption"
        />
      ...
      <div className="bg-[#e6e6e6] pb-10">
        <CommonHeader
          title="Hindi Caption"
          subtitle="Best collection of Hindi Caption in Difrent category"
          slug="hindi-caption"
        />
...
export async function getServerSideProps(context) {
  ...
  const HindiCaption = snapShot.docs.map((docSnap) => {
    return {
      ...docSnap.data(),
  ...
  return {
    props: { HindiCaption }, // will be passed to the page component as props
  };
}

export default HindiCaption;
